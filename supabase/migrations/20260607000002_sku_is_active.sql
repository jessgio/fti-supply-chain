-- Allow SKUs to be marked inactive for forecasting while keeping franchise/bundle mappings.

alter table public.skus
  add column if not exists is_active boolean not null default true;

comment on column public.skus.is_active is
  'When false, SKU stays in franchise/bundle mappings and historical sales, but is excluded from inventory forecast and restock planning.';

create index if not exists skus_is_active_idx on public.skus (is_active)
