-- Scope the demand forecast to "active" SKUs only: those present in the franchise
-- mapping (skus.franchise_id is not null). Unmapped SKUs (one-offs, retired codes,
-- raw materials) are excluded from stock tracking and restock planning.
...
as $$
  ...
  daily as (
    ...
    from (
      -- Direct (non-bundle) sales for mapped SKUs
      select
        s.sku_code,
        sr.sale_date,
        sr.qty_sold
      ...
      -- Bundle sales exploded into their mapped component SKUs
      select
        cs.sku_code,
        sr.sale_date,
