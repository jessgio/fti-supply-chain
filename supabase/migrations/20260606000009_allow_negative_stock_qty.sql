-- WMS Tersedia can be negative when qty is committed/on order (not available)

alter table public.stock_levels
  drop constraint if exists stock_levels_qty_on_hand_check;

alter table public.stock_levels
  add constraint stock_levels_qty_on_hand_check
  check (qty_on_hand >= -9999999999);

-- Include SKUs with negative on-hand in forecast (backorder / on-order)
