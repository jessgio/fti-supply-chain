...
import type { ExtractInboundDnSettings } from "@/types/database";

export default function ExtractInboundDnSettingsPage() {
  const [settings, setSettings] = useState<ExtractInboundDnSettings | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/extract-inbound-delivery-notes/settings");
    ...
    <PageShell className="max-w-3xl">
      ...
      <div>
        ...
        >
          ...
          Back to Extract Code DN Catalog
        </Link>
        <h1 className="text-2xl font-semibold text-stone-900">
          Extract inbound recipient defaults
        </h1>
        <p className="mt-1 text-sm text-stone-600">
          SHIP TO details for extract inbound delivery note PDFs. Separate from the secondary
          packaging CDC recipient used on external delivery notes.
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Manufacturer recipient</CardTitle>
          <CardDescription>
            Default Cosmax manufacturer information. The form&apos;s Penerima field overrides the
