...
import { Loader2, Plus, Search, Settings, Upload, Link2 } from "lucide-react";
...
export default function DeliveryNoteCatalogPage() {
  const [items, setItems] = useState<SecondaryPackagingInboundCosmax[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/delivery-notes/packaging");
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Failed to load catalog.");
      const rows = (data.items ?? []) as SecondaryPackagingInboundCosmax[];
      setItems(rows);
      setDrafts(
        Object.fromEntries(
          rows.map((row) => [
  ...
  async function saveRow(id: string) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/delivery-notes/packaging/${id}`, {
        method: "PATCH",
      ...
      const updated = data.item as SecondaryPackagingInboundCosmax;
  ...
  async function handleCreate(event: React.FormEvent) {
    ...
    try {
      ...
      const created = data.item as SecondaryPackagingInboundCosmax;
  ...
  async function handleCsvUpload() {
    ...
    try {
      ...
      const res = await fetch("/api/delivery-notes/packaging/import", {
        method: "POST",
        body: formData,
      });
      const data = await res.json();
    ...
    <PageShell className="max-w-5xl">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-stone-900">
              Secondary Packaging Inbound to Cosmax
            </h1>
            <p className="mt-1 text-sm text-stone-600">
              Manage the 12-digit item codes and product names used on the external delivery note form.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link href="/dashboard/delivery-notes">
              <Button type="button" variant="outline" size="sm">
                ...
                External portal & history
              </Button>
            </Link>
            <Link href="/dashboard/delivery-notes/settings">
              <Button type="button" variant="outline" size="sm">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            <strong>Description</strong> (product name). Existing codes are updated;
            new codes are added. Excel (.xlsx) is also accepted.
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Item codes must be exactly 12 characters. They appear on the delivery note PDF as Kode Barang.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form
          ...
          >
            ...
            <label className="flex flex-col gap-1.5 text-sm">
              ...
              <Input
                ...
                placeholder="FTI FROM THIS ISLAND … #Unit Box"
                required
              />
            </label>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {items.length} item{items.length === 1 ? "" : "s"} · inactive items are hidden from the external form
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
