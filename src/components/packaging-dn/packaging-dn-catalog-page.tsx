...
export interface PackagingDnCatalogPageProps {
  module: DeliveryNoteModule;
  title: string;
  description: string;
  /** GET list URL (should return inactive items for admin). */
  listUrl: string;
  createUrl: string;
...
export function PackagingDnCatalogPage({
  module,
  title,
  description,
  listUrl,
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const rows = (data.items ?? []) as CatalogItem[];
      setItems(rows);
      setDrafts(
        Object.fromEntries(
          rows.map((row) => [
  ...
  async function handleCreate(event: React.FormEvent) {
    ...
    try {
      ...
      setItems((prev) =>
        [...prev, created].sort((a, b) => a.item_code.localeCompare(b.item_code)),
      );
      setDrafts((prev) => ({
        ...prev,
        [created.id]: {
